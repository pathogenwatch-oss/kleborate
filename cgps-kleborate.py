...
antibiogram_ext = {
    "Bla_Carb_acquired": ["EBL", "BLA"],
    "Bla_ESBL_acquired": ["BLA"],
    "Bla_ESBL_inhibR_acquired": ["EBL", "BLA"],
}
...
def parse_kleborate(
    raw: dict[str, str], amr_dict: dict[str, dict[str, str]]
) -> dict[str, Any]:
    # print(str(raw), file=sys.stderr)
    result: dict[str, Any] = {
        "modules": [],
        "amr": {
            "profile": {},
    ...
    for key, value in raw.items():
        if key == "strain":
            continue
        field_name:str  = key.split("__")[-1]
        module_name: str = "__".join(key.split("__")[0:-1])
        # print(f"Module: {module_name}, Field: {field_name}, Value: {value}", file=sys.stderr)
        for group in modules.keys():
            if module_name in modules[group]:
                ...
                if group == "amr_modules":
                    if field_name in [
                        ...
                        break
                    if field_name.startswith( "Ciprofloxacin"):
                        if field_name == "Ciprofloxacin_prediction":
                            result.update({
                                "Ciprofloxacin_prediction": raw["klebsiella_pneumo_complex__cipro_prediction__Ciprofloxacin_prediction"],
                                "Ciprofloxacin_profile_support": raw["klebsiella_pneumo_complex__cipro_prediction__Ciprofloxacin_profile_support"],
                                "Ciprofloxacin_profile": raw["klebsiella_pneumo_complex__cipro_prediction__Ciprofloxacin_profile"],
                                "Ciprofloxacin_MIC_prediction": raw["klebsiella_pneumo_complex__cipro_prediction__Ciprofloxacin_MIC_prediction"],
                            })
                        break
                    result["amr"]["classes"][field_name] = value
                    phenotype = amr_dict[field_name]
                    tags = [phenotype["key"]]
                    if field_name in antibiogram_ext:
                        tags.extend(antibiogram_ext[field_name])
                    for tag in tags:
                        ...
                        if value != "-":
                            ...
                            if result["amr"]["profile"][tag]["matches"] == "-":
                                ...
                                result["amr"]["profile"][tag][
                                    "matches"
                                ] = f'{result["amr"]["profile"][tag]["matches"]};{value}'
                    ...
                    elif field_name.startswith("virulence") or field_name.startswith(
                        "spurious"
                    ):
            ...
            print(
                f"Key: {key}, Module: {module_name}, Field: {field_name}, Value: {value}",
                file=sys.stderr,
            )
...
def main(
    assembly_file: Annotated[
        Path, typer.Argument(help="Assembly file in fasta format")
    ],
    species: Annotated[
        Profile,
        typer.Argument(case_sensitive=False, help="Specify the Kleborate module set."),
    ],
    amr_json: Annotated[
        Path,
        typer.Option(
    ...
    if (
        not Path(kleborate_version_file).is_file()
        or not Path(code_version_file).is_file()
    ):
        print(
            "Error: Kleborate version and code version files not found", file=sys.stderr
        )
        sys.exit(1)

    with open(kleborate_version_file, "r") as v_fh, open(
        code_version_file, "r"
    ) as c_fh:
    ...
    # Deal with a missing result file.
    if not Path(output_filename).is_file():
        other_filenames = [
            f"/tmp/{config.output_filename}" for config in configs.values()
        ]
        result = {}
    ...
    # Read result files and write as a JSON blob
    with open(output_filename, "r") as result_fh, open(amr_json, "r") as js_fh:
        amr_dict = {
            f"{record['kleborateCode']}_{extension}": record
            for record in json.load(js_fh)
